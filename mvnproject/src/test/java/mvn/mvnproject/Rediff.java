...
public class Rediff {
	...
	public void setExtent() {
		...
		htmlReporter = new ExtentHtmlReporter(System.getProperty("user.dir") + "/Test-Reports/myRediffReport.html");
		//"C:\\worksoft\\test"
		//Create an instance of the class
		//Where to create the HTML report
		
		//htmlReporter.
		htmlReporter.config().setDocumentTitle("Rediff Reports "); // Tile of report - you can change
	...
	// Test1
	@Test
	public void ReediffTitleTest() {
		test = extent.createTest("ReediffTitleTest");
		...
		Assert.assertEquals(title, "Rediff.com");
		//above assert would fail since title is different
	}

	// Test2
	@Test
	public void RediffSignUpVisibleTest() {
		test = extent.createTest("RediffSignUpVisibleTest");
		boolean b = driver.findElement(By.xpath("//a[contains(text(),'Sign in')]")).isDisplayed();
		test.log(Status.INFO, "test.log - INFO  - Checking the Sign Up Link");
		Assert.assertTrue(b);
	}
	...
	public void RediffLoginTest() {
		test = extent.createTest("RediffLoginTest");
	...
	public void tearDown(ITestResult result) throws IOException {
		...
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.FAIL, "TEST CASE FAILED IS " + result.getThrowable()); // to add error/exception in extent
																					// report
			String screenshotPath = Rediff.getScreenshot(driver, result.getName());
