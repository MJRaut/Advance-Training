		...
		if (prop == null) {
			...
			try {
				FileInputStream file = new FileInputStream(System.getProperty("user.dir")
						+ "//src//test//resources//objectRepository/projectConfig2.properties");
				prop.load(file);
			} catch (Exception e) {
...
// zohoLoginTextBox_ClassName
	...
	*/
	...
	/****************** Clear Field ***********************/
	// zohoLoginTextBox_ClassName
	public void ClearTextField(String xpathKey) {
		try {
			//System.out.println("xpathKey is:"+xpathKey);
			getElement(xpathKey).clear();
			reportPass(xpathKey + " : Element Cleared Successfully");
		} catch (Exception e) {
			reportFail(e.getMessage());
		}
	}
